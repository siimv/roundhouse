...
BEGIN
	...
	IF t_exists = 0 THEN
       ...
       -- alter table ' || lower(schName) || '.' || lower(tblName) || ' OWNER TO postgres;
        GRANT SELECT ON TABLE ' || lower(schName) || '.' || lower(tblName) || ' TO public;
        --GRANT ALL ON TABLE ' || lower(schName) || '.' || lower(tblName) || ' TO t_user;';
	END IF;	
END;
$$ LANGUAGE 'plpgsql';
